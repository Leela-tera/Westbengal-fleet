...
class Helper
{
    ...
    public static function registermail($user,$password){
        $to_email = $user->email; 
        Mail::send('emails.welcome_register' , [ 'user' => $user, 'password' => $password] , function($message) use ($to_email) {  
                           $message->to($to_email); 
                           $message->subject('Welcome');
                           $message->from(env('MAIL_FROM_ADDRESS','dinesh@appoets.com'), env('MAIL_FROM_NAME','FieldTracking')); 
                        });
