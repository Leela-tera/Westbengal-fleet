...
<script>
    ...
    function initMap() {
        map = new google.maps.Map(document.getElementById('map'), {
            center: {lat: 17.4302108, lng: 78.4063956},
            zoom: 8,
            minZoom: 1
        });
    ...
    function ajaxMapData() {
        map.clearOverlays();
        $.ajax({
            url: '/admin/trackmap/ajax?district_id='+ districtdata +'&from_date='+fromdate+'&to_date='+todate,
            dataType: "JSON",
    ...
    function addMarkerToMap(element, index) {
    	var baddress = element.address?element.address:(element.provider.latitude + "," + element.provider.longitude);
        marker = new google.maps.Marker({
            position: {
                lat: element.provider.latitude,
                lng: element.provider.longitude
            },
