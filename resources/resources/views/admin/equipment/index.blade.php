        ...
        <div class="col-md-12" style="height:50px;color:red;">
            ...
            <table class="table row-bordered dataTable nowrap display" id="table-5" style="width:100%">
                <thead>
                    <tr>
                        <th>@lang('admin.id')</th>
                        <th>@lang('admin.equipment.equipment_name')</th>
                        <th>@lang('Service Type')</th>
                        <th>@lang('Current Assignee')</th>
                        <th>@lang('Service Number')</th>
                        <th>@lang('Service Model')</th>
                        <th>@lang('District')</th>
                        <th>@lang('Block')</th>
                ...
                @php($page++)
                    <tr>
                        <td>{{ $page }}</td>
                        <td class="font-weight-bold">{{ $equipment->name }}</td>
                        <td>{{ $equipment->service_type }}</td>
                        <td>{{ $equipment->first_name }} {{ $equipment->last_name }}</td>
                        <td>{{ $equipment->service_number }}</td>
                        <td>{{ $equipment->service_model }}</td>
                        <td>{{ $equipment->district_name }}</td>
                        <td>{{ $equipment->block_name }}</td>
                        <td>
                            <div class="btn-group" style="width:200px">
                                ...
                                <form action="{{ route('admin.equipment.destroy', $equipment->id) }}" method="POST">
                                    ...
                                    <button class="btn btn-danger b-a-radius-0-5 pull-left mr-1" onclick="return confirm('Are you sure you want to delete this Equipment?')"><i class="fa fa-trash"></i> @lang('admin.delete')</button>
                                </form>
