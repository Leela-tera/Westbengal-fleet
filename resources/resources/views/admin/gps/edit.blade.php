...
<div class="content-area py-1">
    <div class="container-fluid">
    	<div class="box box-block bg-white">
			...
			<h5 style="margin-bottom: 2em;">@lang('admin.gp.update_gp')</h5>

      <form class="form-horizontal" action="{{route('admin.gps.update', $gp->id )}}" method="POST" enctype="multipart/form-data" role="form">
            	{{csrf_field()}}
		...
		<div class="top-cs box-block shadow-gray mb-3">
        	...
        	<div class="form-group row">
						<div class="col-sm-10 col-md-10">
							...
							<input class="form-control select-box" type="text" value="{{ $gp->gp_name }}" name="gp_name"  id="gp_name" placeholder="GP Name">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<label for="district" class="col-form-label">Districts</label>
							<select class="form-control select-box" name="district" id="district">
								...
								<option value="{{$dist->id}}" {{ $dist->id == $gp->district_id ? 'selected' : '' }}>{{$dist->name}}</option>
								@endforeach
					...
					<div class="form-group row">
						...
						<label for="block" class="col-form-label">Block</label>
							<select class="form-control select-box" name="block" id="block">
								...
								<option value="{{$block->id}}" {{ $block->id == $gp->block_id ? 'selected' : '' }}>{{$block->name}}</option>
								@endforeach
					...
					<div class="form-group row">
						...
						<label for="district" class="col-form-label">Zonal Manager</label>
							<select class="form-control select-box" name="zone" id="zone">
								...
								@foreach($zonals as $dist)
								<option value="{{$dist->id}}" {{ $dist->id == $gp->zonal_id ? 'selected' : '' }}>{{$dist->Name}}</option>
								@endforeach
	        ...
	        @endif
					...
					<div class="form-group row">
						<div class="col-sm-10 col-md-10">
							...
							<select class="form-control select-box" name="provider" id="provider" >
								...
								<option value="{{ $provider->first_name }} {{ $provider->last_name }}" {{ $provider->name == $gp->provider ? 'selected' : '' }}>{{ $provider->name }}</option>
								@endforeach									
							</select>
						</div>
					</div>
					...
					<div class="form-group row">
						<div class="col-sm-10 col-md-10">
							...
							<input class="form-control select-box" type="number" value="{{ $gp->contact_no }}" name="contact"  id="contact" placeholder="Provider Contact">
						</div>
					</div>
					<div class="form-group row">
						<div class="col-sm-10 col-md-10">
							...
							<input class="form-control select-box" type="text" value="{{ $gp->lgd_code }}" name="lgd_code"  id="lgd_code" placeholder="LGD Code">
						</div>
					</div>
					<div class="form-group row">
						<div class="col-sm-10 col-md-10">
							...
							<input class="form-control select-box" type="text" value="{{ $gp->phase }}" name="phase"  id="phase" placeholder="Phase">
						</div>
					</div>
					<div class="form-group row">
						<div class="col-sm-10 col-md-10">
							...
							<input class="form-control select-box" type="text" value="{{ $gp->latitude }}" name="latitude" id="latitude" placeholder="Latitude">
						</div>
					</div>
					<div class="form-group row">
						<div class="col-sm-10 col-md-10">
							...
							<input class="form-control select-box" type="text" value="{{ $gp->longitude }}" name="longitude" id="longitude" placeholder="Longitude">
						</div>
					</div>
					<div class="form-group row">
						<label for="zipcode" class="col-xs-12 col-form-label"></label>
						<div class="col-xs-12 mt-2">
							<button type="submit" class="btn btn-primary btn-cstm pull-right ">@lang('admin.gp.update_gp')</button>
